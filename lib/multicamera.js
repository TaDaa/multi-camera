...
export class MultiCamera {
    ...
    _hidePhotoOverlay() {
        const image = this._activePhoto, { wrapper: original } = image || { wrapper: undefined }, wrapper = this._showingPhotoOverlay !== false ? this._showingPhotoOverlay.wrapper : this._showingPhotoOverlay, rect = original && original.getBoundingClientRect(), elements = this._elements;
        if (!wrapper) {
            return;
        }
    ...
    _centerPhotoOverlay() {
        const dimensions = this.getScreenDimensions(), cloneImage = this._showingPhotoOverlay;
